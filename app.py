...
def home():
    ...
    price1 = get_price('AMZN')
    price2 = get_price('GOOG')
    price3 = get_price('TSLA')
    price1['cur_price'] = format_num(
        price1['cur_price'])
    price2['cur_price'] = format_num(
        price2['cur_price'])
    price3['cur_price'] = format_num(
        price3['cur_price'])
    return render_template('home.html', price1=price1, price2=price2, price3=price3)
...
def user_details(username):
    ...
    if user:
        ...
        for stock in covered_stocks:
            stock['date'] = datetime.date.strftime(
                stock['date'], "%m/%d/%Y")  # in mm/dd/yyyy
            stock['target'] = "{:,.2f}".format(
                stock['target'])  # in $ amt format
...
def user_forecast_details(username, ticker):
    ...
    if user and forecasts:
        ...
        for forecast in forecasts:
            forecast['date'] = datetime.date.strftime(
                forecast['date'], "%m/%d/%Y")
...
def display_forecast(forecast_id):
    ...
    if forecast:
        ...
        forecast_dict['gross_income'] = [sum(i) for i in zip(
            forecast_dict['revenue'], forecast_dict['cogs'])]
        forecast_dict['ebitda'] = [sum(i) for i in zip(
            forecast_dict['gross_income'], forecast_dict['opex'])]
        forecast_dict['operating_income'] = [sum(i) for i in zip(
            forecast_dict['ebitda'], forecast_dict['depreciation'])]
        forecast_dict['ebt'] = [sum(i) for i in zip(
            forecast_dict['operating_income'], forecast_dict['other'])]
        forecast_dict['last_eps'] = forecast_dict['net_income'][len(
            forecast_dict['net_income']) - 1] / forecast_dict['shares_out']
        forecast_dict['date'] = datetime.date.strftime(
            forecast_dict['date'], "%m/%d/%Y")
        ...
        forecast_dict['price'] = forecast_dict['last_eps'] * \
            forecast_dict['multiple']
        forecast_dict['dividend_per_share'] = sum(
            forecast_dict['dividend']) / forecast_dict['shares_out']
        # the curprice will be based on the stock's current price in the market
        forecast_dict['return'] = (
            forecast_dict['target'] / price['cur_price']) ** (1/4) - 1
        ...
        for k in forecast_dict:
            if k in ['revenue', 'cogs', 'opex', 'depreciation', 'other', 'tax', 'net_income', 'dividend', 'ebitda', 'operating_income', 'ebt', 'gross_income']:
                for i in range(len(forecast_dict[k])):
                    forecast_dict[k][i] = "${:,.0f}".format(
                        forecast_dict[k][i] / 1000000)  # show as $ amt in millions
            elif k == 'return':
                forecast_dict[k] = "{:,.2f}%".format(
                    forecast_dict[k]*100)  # show as %
            elif k in ['multiple', 'last_eps', 'price', 'dividend_per_share', 'target']:
                forecast_dict[k] = "{:,.2f}".format(
                    forecast_dict[k])  # show as $ amt
...
def update_weighting(username, ticker):
    ...
    if forecasts and user:
        ...
        for forecast_dict in forecasts:
            ...
            # get the new weighting from form
            new_weighting = float(request.form[str(forecast_dict['id'])])
